...
function collectFormData() {
    ...
    if (areaBody) {
        fd._area_rows = Array.from(areaBody.rows).map(row => {
            ...
            return {
                particulars : (row.cells && row.cells[0]) ? row.cells[0].textContent.trim() : '',
    ...
    // 6. Deviation table rows (_deviation_rows)
    const devBody = document.querySelector('#deviationTable tbody');
    if (devBody) {
        fd._deviation_rows = Array.from(devBody.rows).map(row => {
            const ins = row.querySelectorAll('input');
            return {
                flat_no           : ins[0]?.value || '',
                as_per_deed       : ins[1]?.value || '',
                as_per_plan       : ins[2]?.value || '',
                as_per_physical   : ins[3]?.value || '',
                deviation_sft     : ins[4]?.value || '',
                deviation_percent : ins[5]?.value || '',
            };
        });
    }
...
function populateFormData(fd) {
    ...
    // 6. Deviation table rows
    if (Array.isArray(fd._deviation_rows)) {
        const devBody = document.querySelector('#deviationTable tbody');
        if (devBody) {
            devBody.innerHTML = '';
            fd._deviation_rows.forEach(r => {
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td><input type="text" value="${r.flat_no || ''}" placeholder="Flat No."></td>
                    <td><input type="text" class="calc-a" value="${r.as_per_deed || ''}" placeholder="0"></td>
                    <td><input type="text" class="calc-b" value="${r.as_per_plan || ''}" placeholder="0"></td>
                    <td><input type="text" class="calc-c" value="${r.as_per_physical || ''}" placeholder="0"></td>
                    <td><input type="text" class="calc-d" value="${r.deviation_sft || ''}" readonly></td>
                    <td><input type="text" class="calc-e auto-calc" value="${r.deviation_percent || ''}" readonly></td>
                    <td class="action-cell">
                        <button type="button" class="btn btn-outline btn-sm delete-row" onclick="removeDeviationRow(this)">Delete</button>
                    </td>
                `;
                devBody.appendChild(tr);
                // Re-bind calculations if the function is available globally
...
function calculateDeviation(row) {
    const bIn = row.querySelector('.calc-b'); // Area as per Plan
    const cIn = row.querySelector('.calc-c'); // Area as per Physical
    const dOut = row.querySelector('.calc-d'); // Deviation Sft
    const eOut = row.querySelector('.calc-e'); // Deviation %
    if (!bIn || !cIn || !dOut || !eOut) return;

    const b = parseFloat(bIn.value.replace(/,/g, '')) || 0;
    const c = parseFloat(cIn.value.replace(/,/g, '')) || 0;

    const diff = c - b;
    const pct = b !== 0 ? (diff / b) * 100 : 0;

    dOut.value = diff.toFixed(2);
    eOut.value = pct.toFixed(2);
}

function bindRowCalculation(row) {
    row.querySelectorAll('.calc-b, .calc-c').forEach(el => {
        el.addEventListener('input', () => calculateDeviation(row));
    });
}

function removeDeviationRow(btn) {
    btn.closest('tr').remove();
}

// Global listener for auto-calc (handles existing and new rows via delegation)
document.addEventListener('input', (e) => {
    const row = e.target.closest('#deviationTable tbody tr');
    if (row && (e.target.classList.contains('calc-b') || e.target.classList.contains('calc-c'))) {
